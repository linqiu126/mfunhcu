...
include_once "../database/db_windspeed.class.php";

class class_windSpeed_service
{
    ...
    private function wx_windspeed_req_process( $deviceId, $content)
    {
        ...
        $sDbObj = new class_windspeed_db();
        $sDbObj->db_windSpeed_data_save($deviceId,$devCode,$ntimes,$windSpeed,$gps);
    ...
    private function hcu_windspeed_req_process( $deviceId,$statCode,$content)
    {
        ...
        $sDbObj = new class_windspeed_db();
        $sDbObj->db_windSpeed_data_save($deviceId, $sensorId, $timeStamp, $report,$gps);
        //$wxDbObj->db_AirPmDataInfo_delete_3monold($fromuser, $deviceid, $boxid,90);  //remove 90 days old data.

        //更新分钟测量报告聚合表
        $sDbObj->db_minreport_update_windspeed($deviceId,$statCode,$timeStamp,$report);

        //更新数据精度格式表
        $format = $report["format"];
        $cDbObj = new class_common_db();
        $cDbObj->db_dataformat_update_format($deviceId,"T_windspeed",$format);
        //更新瞬时测量值聚合表
        $cDbObj->db_currentreport_update_value($deviceId, $statCode, $timeStamp,"T_windspeed", $report);
