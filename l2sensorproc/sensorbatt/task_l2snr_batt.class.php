...
class classTaskL2snrBatt
{
    ...
    public function func_batt_data_process($platform, $devCode, $statCode, $content)
    {
        ...
        if ($length != strlen($content)) {
            return "ERROR FHYS_BATT: message length invalid";  //消息长度不合法，直接返回
        }

        $opt_key = hexdec($msgHead['Cmd']) & 0xFF;

        if ($opt_key == MFUN_HCU_OPT_FHYS_BATTSTAT_IND){
            $data = substr($content, MFUN_HCU_MSG_HEAD_LENGTH, 2);
            $data = hexdec($data) & 0xFF;
            $classDbiL2snrBatt = new classDbiL2snrBatt();
            $resp = $classDbiL2snrBatt->dbi_hcu_batt_status_update($devCode, $statCode, $data);
        }
        elseif ($opt_key == MFUN_HCU_OPT_FHYS_BATTLEVEL_IND){
            $data = substr($content, MFUN_HCU_MSG_HEAD_LENGTH, 2);
            $data = hexdec($data) & 0xFF;
            $classDbiL2snrBatt = new classDbiL2snrBatt();
            $resp = $classDbiL2snrBatt->dbi_hcu_batt_level_process($devCode, $statCode, $data);
        }
        else
            $resp = "ERROR FHYS_BATT: Invalid Operation Command";
    ...
    public function mfun_l2snr_batt_task_main_entry($parObj, $msgId, $msgName, $msg)
    {
        ...
        //具体处理函数
        if ($msgId == MSG_ID_L2SDK_HCU_TO_L2SNR_BATT)
        {
