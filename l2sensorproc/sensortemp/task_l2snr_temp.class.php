...
class classTaskL2snrTemp
{
    ...
    public function mfun_l2snr_temp_task_main_entry($parObj, $msgId, $msgName, $msg)
    {
        ...
        if (empty($msg) == true) {
            ...
            $loggerObj->mylog($project,"NULL","MFUN_TASK_ID_L2DECODE_HUITP","MFUN_TASK_ID_L2SNR_TEMP",$msgName,$log_content);
            return false;
        }
        else{
            //解开消息
        ...
        if ($msgId == HUITP_MSGID_uni_temp_data_report)
        {
            $dbiL2snrTempObj = new classDbiL2snrTemp();
            $respHuitpMsg = $dbiL2snrTempObj->dbi_huitp_msg_uni_temp_data_report($devCode, $statCode, $content);

            //发送HUITP_MSGID_uni_temp_data_confirm
            if (!empty($respHuitpMsg)) {
                $msg = array("project" => $project,
                    "devCode" => $devCode,
                    "respMsg" => HUITP_MSGID_uni_temp_data_confirm,
                    "content" => $respHuitpMsg);
                if ($parObj->mfun_l1vm_msg_send(MFUN_TASK_ID_L2SENSOR_NOISE,
        ...
        if (!empty($resp)) {
            ...
            $loggerObj->mylog($project,$devCode,"MFUN_TASK_ID_L2SNR_TEMP","MFUN_TASK_ID_L2ENCODE_HUITP",$msgName,$log_content);
        }
