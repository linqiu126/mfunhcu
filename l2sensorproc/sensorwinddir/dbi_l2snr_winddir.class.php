...
class classDbiL2snrWinddir
{
    ...
    private function dbi_l2snr_winddir_update($mysqli,$devCode,$timeStamp,$winddirValue)
    {
    ...
    //更新传感器分钟聚合表
    private function dbi_l2snr_winddir_minreport_update($mysqli,$devCode,$statCode,$timeStamp,$winddirValue)
    {
        $reportdate = date("Y-m-d", $timeStamp);
        $stamp = getdate($timeStamp);
        $hourminindex = intval(($stamp["hours"] * 60 + floor($stamp["minutes"]/MFUN_HCU_AQYC_TIME_GRID_SIZE)));
    ...
    //更新传感器当前报告聚合表
    private function dbi_l2snr_winddir_currentreport_update($mysqli,$devCode,$statCode,$timeStamp,$winddirValue)
    {
    ...
    public function dbi_huitp_msg_uni_winddir_data_report($devCode, $statCode, $content)
    {
        ...
        //保存记录到对应l2snr表
        $result1 = $this->dbi_l2snr_winddir_update($mysqli,$devCode,$timeStamp,$winddirValue);
        //更新分钟测量报告聚合表
        $result2 = $this->dbi_l2snr_winddir_minreport_update($mysqli,$devCode,$statCode,$timeStamp,$winddirValue);
        //更新瞬时测量值聚合表
        $result3 = $this->dbi_l2snr_winddir_currentreport_update($mysqli,$devCode,$statCode,$timeStamp,$winddirValue);
