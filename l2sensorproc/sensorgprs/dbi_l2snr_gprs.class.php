...
class classDbiL2snrGprs
{
    public function dbi_hcu_gprs_status_update($devCode, $statCode, $data)
    {
        ...
        if (($result != false) && ($result->num_rows)>0)
        {
            ...
            $ctrl_key = $apiL2snrCommonServiceObj->byte2string(MFUN_HCU_CMDID_FHYS_GPRS);
            $opt_key = $apiL2snrCommonServiceObj->byte2string(MFUN_HCU_OPT_FHYS_GPRSSTAT_RESP);
    ...
    public function dbi_hcu_sig_level_process($devCode, $statCode, $data)
    {
        ...
        if (($result != false) && ($result->num_rows)>0)
        {
            ...
            $apiL2snrCommonServiceObj = new classApiL2snrCommonService();
            $ctrl_key = $apiL2snrCommonServiceObj->byte2string(MFUN_HCU_CMDID_FHYS_GPRS);
            $opt_key = $apiL2snrCommonServiceObj->byte2string(MFUN_HCU_OPT_FHYS_SIGLEVEL_RESP);
