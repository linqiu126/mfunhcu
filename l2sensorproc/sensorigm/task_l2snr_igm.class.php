...
class classTaskL2snrIgm
{
    ...
    function func_igm_cj188_ul_read_data_process($user)
    {
        $igmObj = new classDbiL2snrIgm(); //初始化一个UI DB对象
        $jsonencode = json_encode($igmObj);
        return $jsonencode;
    }

    function func_igm_cj188_ul_read_key_ver_process($user)
    {
        $igmObj = new classDbiL2snrIgm(); //初始化一个UI DB对象
        $jsonencode = json_encode($igmObj);
        return $jsonencode;
    ...
    function func_igm_cj188_ul_read_address_process($user)
    {
        $igmObj = new classDbiL2snrIgm(); //初始化一个UI DB对象
        $jsonencode = json_encode($igmObj);
        return $jsonencode;
    ...
    public function mfun_l2snr_igm_task_main_entry($parObj, $msgId, $msgName, $msg)
    {
        ...
        if ($msgId == MSG_ID_L2SDK_NBIOT_STD_CJ188_TO_L2SNR_IGM_READ_DATA)
        {
            ...
            //具体处理函数
            $resp = $this->func_igm_cj188_ul_read_data_process($user);
        ...
        elseif ($msgId == MSG_ID_L2SDK_NBIOT_STD_CJ188_TO_L2SNR_IGM_READ_KEY_VER)
        {
            ...
            if (isset($msg["user"])) $user = $msg["user"]; else  $user = "";
            //具体处理函数
            $resp = $this->func_igm_cj188_ul_read_key_ver_process($user);
            $project = MFUN_PRJ_NB_IOT_IGM188;
        }
        ...
        elseif ($msgId == MSG_ID_L2SDK_NBIOT_STD_CJ188_TO_L2SNR_IGM_READ_ADDR)
        {
            ...
            if (isset($msg["user"])) $user = $msg["user"]; else  $user = "";
            //具体处理函数
            $resp = $this->func_igm_cj188_ul_read_address_process($user);
            $project = MFUN_PRJ_NB_IOT_IGM188;
        }
