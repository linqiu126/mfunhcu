...
class classDbiL2snrHsmmp
{
    ...
    */
    ...
    public function dbi_picture_data_save($deviceid, $sensorid, $timestamp, $bindata, $gps)
    {
        ...
        //存储新记录，如果发现是已经存在的数据，则覆盖，否则新增
        $result = $mysqli->query("SELECT * FROM `t_picturedata` WHERE (`deviceid` = '$deviceid' AND `sensorid` = '$sensorid'
                                  AND `reportdate` = '$date' AND `hourminindex` = '$hourminindex')");
        if (($result->num_rows)>0)   //重复，则覆盖
        {
            $result=$mysqli->query("UPDATE `t_picturedata` SET `bindata` = '$bindata',`altitude` = '$altitude',`flag_la` = '$flag_la',`latitude` = '$latitude',`flag_lo` = '$flag_lo',`longitude` = '$longitude'
                    WHERE (`deviceid` = '$deviceid' AND `sensorid` = '$sensorid' AND `reportdate` = '$date' AND `hourminindex` = '$hourminindex')");
        }
        else   //不存在，新增
        {
            $result=$mysqli->query("INSERT INTO `t_picturedata` (deviceid,sensorid,bindata,reportdate,hourminindex,altitude,flag_la,latitude,flag_lo,longitude)
    ...
    public function dbi_latest_picture_data_inqury($deviceid)
    {
        ...
        $result = $mysqli->query("SELECT * FROM `t_picturedata` WHERE `deviceid` = '$deviceid'");
