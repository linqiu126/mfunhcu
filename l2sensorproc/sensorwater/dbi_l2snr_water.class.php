...
class classDbiL2snrWater
{

    public function dbi_hcu_water_status_update($devCode, $statCode, $data)
    {
        //建立连接
        ...
        if (($result->num_rows)>0) {
            $query_str = "UPDATE `t_l3f3dm_fhys_currentreport` SET  `waterstat` = '$status', `createtime` = '$currenttime' WHERE (`devcode` = '$devCode')";
            $result = $mysqli->query($query_str);
        }
        else {
            $query_str ="INSERT INTO `t_l3f3dm_fhys_currentreport` (devcode,statcode,createtime,waterstat) VALUES ('$devCode','$statCode','$currenttime','$status')";
            $result = $mysqli->query($query_str);
        }
        ...
        if (($result != false) && ($result->num_rows)>0)
        {
            ...
            $ctrl_key = $dbiL1vmCommonObj->byte2string(MFUN_HCU_CMDID_FHYS_WATER);
            $opt_key = $dbiL1vmCommonObj->byte2string(MFUN_HCU_OPT_FHYS_WATERSTAT_RESP);
            ...
            if ($socketid != 0){
                ...
                $client->connect();
                $resp = "Sensor water status response send success";
            }
            else{
            ...
            $resp = "Sensor water status response send failure";

        $mysqli->close();
        return $resp;
    }

    public function dbi_hcu_water_alarm_process($devCode, $statCode)
    {
        //建立连接
        $mysqli = new mysqli(MFUN_CLOUD_DBHOST, MFUN_CLOUD_DBUSER, MFUN_CLOUD_DBPSW, MFUN_CLOUD_DBNAME_L1L2L3, MFUN_CLOUD_DBPORT);
        if (!$mysqli) {
        ...
        if (($result != false) && ($result->num_rows)>0)
        {
            ...
            $dbiL1vmCommonObj = new classDbiL1vmCommon();
            $ctrl_key = $dbiL1vmCommonObj->byte2string(MFUN_HCU_CMDID_FHYS_WATER);
            $opt_key = $dbiL1vmCommonObj->byte2string(MFUN_HCU_OPT_FHYS_WATERALARM_RESP);
