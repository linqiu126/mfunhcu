...
class classDbiL2snrHumid
{
    ...
    private function dbi_l2snr_humiddata_update($devCode, $timeStamp, $humidValue)
    {
        ...
        {
            $query_str = "UPDATE `t_l2snr_humiddata` SET `humidity` = '$humidValue' WHERE (`deviceid` = '$devCode' AND `reportdate` = '$reportdate' AND `hourminindex` = '$hourminindex')";
            $result=$mysqli->query($query_str);
        }
        else   //不存在，新增
        {
    ...
    public function dbi_huitp_msg_uni_humid_data_report($devCode, $statCode, $content)
    {
        ...
        //清理超过90天记录的数据
        $result = $this->dbi_l2snr_humiddata_old_delete($devCode, MFUN_AQYC_DATA_SAVE_DURATION_IN_DAYS);  //remove old data.
