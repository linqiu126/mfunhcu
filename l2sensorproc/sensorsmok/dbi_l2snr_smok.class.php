...
class classDbiL2snrSmok
{
    ...
    public function dbi_hcu_smok_status_update($devCode, $statCode, $data)
    {
        ...
        if (($result != false) && ($result->num_rows)>0)
        {
            ...
            $ctrl_key = $dbiL1vmCommonObj->byte2string(MFUN_HCU_CMDID_FHYS_SMOK);
            $opt_key = $dbiL1vmCommonObj->byte2string(MFUN_HCU_OPT_FHYS_SMOKSTAT_RESP);
    ...
    public function dbi_hcu_smok_alarm_process($devCode, $statCode)
    {
        ...
        if (($result != false) && ($result->num_rows)>0)
        {
            ...
            $dbiL1vmCommonObj = new classDbiL1vmCommon();
            $ctrl_key = $dbiL1vmCommonObj->byte2string(MFUN_HCU_CMDID_FHYS_SMOK);
            $opt_key = $dbiL1vmCommonObj->byte2string(MFUN_HCU_OPT_FHYS_SMOKALARM_RESP);
