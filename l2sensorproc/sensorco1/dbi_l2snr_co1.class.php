...
class classDbiL2snrCo1
{
    ...
    public function dbi_co1Data_delete_3monold($deviceid, $sensorid,$days)
    {
        ...
        $result = $mysqli->query("DELETE FROM `t_l2snr_co1data` WHERE ((`deviceid` = '$deviceid' AND `sensorid` ='$sensorid') AND (TO_DAYS(NOW()) - TO_DAYS(`date`) > '$days'))");
        $mysqli->close();
        return $result;
    }
    ...
    public function dbi_minreport_update_co1($devcode,$statcode,$timestamp,$data)
    {
        ...
        if (($result != false) && ($result->num_rows)>0)   //重复，则覆盖
        {
            $result=$mysqli->query("UPDATE `t_l2snr_minreport` SET `co1` = '$co1'
                          WHERE (`devcode` = '$devcode' AND `statcode` = '$statcode' AND `reportdate` = '$date' AND `hourminindex` = '$hourminindex')");
        }
        else   //不存在，新增
        {
            $result=$mysqli->query("INSERT INTO `t_l2snr_minreport` (devcode,statcode,co1,reportdate,hourminindex)
