...
class classDbiL2snrCcl
{
    ...
    public function dbi_huitp_msg_uni_ccl_auth_inq($devCode, $statCode, $data)
    {
        ...
        if (($authRequestType == HUITP_IEID_UNI_CCL_LOCK_AUTH_REQ_TYPE_BLE) AND !empty($bleMacAddr) AND ($auth_check == false))
        {
        ...
        if (($authRequestType == HUITP_IEID_UNI_CCL_LOCK_AUTH_REQ_TYPE_LOCK) AND ($auth_check == false))
        {
