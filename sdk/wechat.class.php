 ...
 *  @author  dodge <dodgepudding@gmail.com> ZJL Update
...
include_once "../sdk/wx_iot.class.php";
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    //Layer 2.1 SDK01 orignal design
    //官方公号轻量级SDK01，用户普通公号接入到微信平台
    //验证签名
    public function valid_sdk01()
    {
        $echoStr = $_GET["echostr"];
...
*********************************************************************************************************************/
    ...
    //回复MSG1.8社交功能消息
    public function transmitSocialMessage($object)
    {
        $xmlTpl = "<xml>
            <ToUserName><![CDATA[%s]]></ToUserName>
            <FromUserName><![CDATA[%s]]></FromUserName>
            ...
            <HardWare>
            <HardWare.MessageView><![CDATA[myrank]]></HardWare.MessageView>
            <HardWare.MessageAction><![CDATA[ranklist]]></HardWare.MessageAction></HardWare>
            <FuncFlag>0</FuncFlag>
            </xml>";
        $result = sprintf($xmlTpl, $object->FromUserName, $object->ToUserName, time());
        return $result;
            ...
            case "CLICK":
                switch ($object->EventKey)
                {
                    ...
                    case "CLICK__EMC_HIS":
                        ...
                        $content[] = array("Title"=>"辐射累积值", "Description"=>"", "PicUrl"=>"", "Url" =>"http://mfuncard.sinaapp.com/wechat/jsp/EmcAllData.php?id=".$object->FromUserName);
                        $content[] = array("Title"=>"PM2.5累积值", "Description"=>"", "PicUrl"=>"", "Url" =>"http://mfuncard.sinaapp.com/wechat/jsp/PMAllData.php?id=".$object->FromUserName);
                        $content[] = array("Title"=>"近一月辐射累计值", "Description"=>"", "PicUrl"=>"", "Url" =>"http://mfuncard.sinaapp.com/wechat/jsp/EmcAccumulationInfo_1M.php?id=".$object->FromUserName);
                        $content[] = array("Title"=>"近三月辐射累计值", "Description"=>"", "PicUrl"=>"", "Url" =>"http://mfuncard.sinaapp.com/wechat/jsp/EmcAccumulationInfo_3M.php?id=".$object->FromUserName);
                        break;
