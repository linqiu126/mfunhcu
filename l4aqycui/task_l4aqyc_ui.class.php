...
class classTaskL4aqycUi
{
    ...
    public function mfun_l4aqyc_ui_task_main_entry($parObj, $msgId, $msgName, $msg)
    {
        ...
        if (empty($msg) == true) {
            ...
            $loggerObj->mylog($project,"NULL","MFUN_TASK_ID_L1VM","MFUN_TASK_ID_L4AQYC_UI",$msgName,$log_content);
        ...
        switch($action)
        {
            ...
            case "ProjPoint":   //查询所有监控点列表
                ...
                $input = array("action" => $action, "type" => $type,"user" => $user,"body" => $body);
                $parObj->mfun_l1vm_msg_send(MFUN_TASK_ID_L4AQYC_UI, MFUN_TASK_ID_L3APPL_FUM2CM, MSG_ID_L4AQYCUI_TO_L3F2_ALLPROJPOINT, "MSG_ID_L4AQYCUI_TO_L3F2_ALLPROJPOINT",$input);
                break;
            ...
            case "PointProj": //查询该项目下面对应监控点列表
                ...
                $input = array("action" => $action, "type" => $type,"user" => $user,"body" => $body);
                $parObj->mfun_l1vm_msg_send(MFUN_TASK_ID_L4AQYC_UI, MFUN_TASK_ID_L3APPL_FUM2CM, MSG_ID_L4AQYCUI_TO_L3F2_ONEPROJPOINT, "MSG_ID_L4AQYCUI_TO_L3F2_ONEPROJPOINT",$input);
                break;
            ...
            case "GetVideoList": //获取指定站点指定时间段内的所有视频文件列表
                ...
                $input = array("action" => $action, "type" => $type,"user" => $user,"body" => $body);
                $parObj->mfun_l1vm_msg_send(MFUN_TASK_ID_L4AQYC_UI, MFUN_TASK_ID_L3APPL_FUM4ICM, MSG_ID_L4AQYCUI_TO_L3F4_HSMMPLIST, "MSG_ID_L4AQYCUI_TO_L3F4_HSMMPLIST",$input);
                break;
        ...
        if (!empty($resp)) {
            ...
            $loggerObj->mylog($project,$user,"MFUN_TASK_ID_L4AQYC_UI","NULL",$msgName,$log_content);
