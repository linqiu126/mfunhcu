...
class classL1MainEntrySocketListenServer
{
    ...
    public function __construct() {
        ...
        $this->swoole_socket_serv = new swoole_server("0.0.0.0", MFUN_SWOOLE_SOCKET_STDXML_TCP_HCUPORT,SWOOLE_PROCESS,SWOOLE_SOCK_TCP);
        ...
        $this->swoole_socket_serv->set(array(
            ...
            'max_request' => 2000,  //=2000,每个worker进程允许处理的最大任务数,每个worker进程在处理完max_request个请求后就会自动重启。设置该值的主要目的是为了防止worker进程处理大量请求后可能引起的内存溢出
            'max_conn' => 1000, //服务器允许维持的最大TCP连接数。超过此数量后，新进入的连接将被拒绝。
            'ipc_mode' => 1, //设置进程间的通信方式,1 => 使用unix socket通信/2 => 使用消息队列通信/3 => 使用消息队列通信，并设置为争抢模式
            'dispatch_mode' => 3,  //=2 指定数据包分发策略,1 => 轮循模式，收到会轮循分配给每一个worker进程/2 => 固定模式，根据连接的文件描述符分配worker。这样可以保证同一个连接发来的数据只会被同一个worker处理/3 => 抢占模式，主进程会根据Worker的忙闲状态选择投递，只会投递给处于闲置状态的Worker
        ...
        $stdxml_tcp_uiport = $this->swoole_socket_serv->addlistener("0.0.0.0", MFUN_SWOOLE_SOCKET_STDXML_TCP_UIPORT, SWOOLE_SOCK_TCP);
        ...
        /*$huitpxml_tcp_hcuport->set(array(
            //'open_tcp_nodelay' => true, //开启后TCP连接发送数据时会无关闭Nagle合并算法，立即发往客户端连接。在某些场景下，如http服务器，可以提升响应速度。
            'open_eof_check' => true, //打开eof检测功能。当数据包结尾是指定的package_eof 字符串时才会将数据包投递至Worker进程，否则会一直拼接数据包直到缓存溢出或超时才会终止。一旦出错，该连接会被判定为恶意连接，数据包会被丢弃并强制关闭连接。
            'package_eof ' => 'AAA', //设置EOF字符串, package_eof最大只允许传入8个字节的字符串
            'open_eof_split'=>true)
        );*/
        ...
        //CCL图片传输端口
        $huitpxml_tcp_picport = $this->swoole_socket_serv->addlistener("0.0.0.0", MFUN_SWOOLE_SOCKET_DATA_STREAM_TCP, SWOOLE_SOCK_TCP);
    ...
    public function stdxml_tcp_hcuport_onConnect($swoole_socket_serv, $fd, $from_id ) {
        echo date('Y/m/d H:i:s', time())." ";
        echo "stdxml_tcp_hcuport_onConnect: Client [{$fd}] connected, port_id={$swoole_socket_serv->port}, from_reactor_id=$from_id".PHP_EOL;
    ...
    public  function stdxml_tcp_uiport_onConnect($stdxml_tcp_uiport, $fd, $from_id)
    {
        echo date('Y/m/d H:i:s', time())." ";
        echo "stdxml_tcp_uiport_onConnect: Client [{$fd}] connected, from_port=[{$stdxml_tcp_uiport->port}]".PHP_EOL;
    }
    ...
    public function huitpxml_tcp_hcuport_onConnect($huitpxml_tcp_hcuport, $fd, $from_id)
    {
        echo date('Y/m/d H:i:s', time())." ";
        echo "huitpxml_tcp_hcuport_onConnect: Client [{$fd}] connected, from_port=[{$huitpxml_tcp_hcuport->port}]".PHP_EOL;
    }
