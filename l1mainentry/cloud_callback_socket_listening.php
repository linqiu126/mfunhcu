...
class classL1MainEntrySocketListenServer
{
    ...
    public function __construct() {
        ...
        $swoole_Server  = new swoole_http_server("0.0.0.0", MFUN_SWOOLE_SOCKET_HUITPJSON_HTTP);
        ...
        /********************Swoole server公共处理函数，Master进程/Manager进程/Worker进程/Task进程************************/
        //Master
        //进程回调函数 onStart/onShutdown/onMasterConnect/onMasterClose/onTimer
        $swoole_Server->on('Start', array($this, 'swoole_server_onStart'));  //启动server，监听所有TCP/UDP端口
        //manager
        //进程回调函数 onManagerStart/onManagerStop
        $swoole_Server->on('ManagerStart', array($this, 'swoole_server_onManagerStart'));
        //Worker
        //进程回调函数 onWorkerStart/onWorkerStop/onConnect/onClose/onReceive/onTimer/onFinish
        $swoole_Server->on('WorkerStart', array($this, 'swoole_server_onWorkerStart'));
        $swoole_Server->on('WorkerStop', array($this, 'swoole_server_onWorkerStop'));
        $swoole_Server->on('Finish', array($this, 'swoole_server_onFinish'));
        //Task
        //进程回调函数 onTask/onWorkerStart
        $swoole_Server->on('Task', array($this, 'swoole_server_onTask'));


        //huitpjson_tcp_port, 传送HUITP JSON消息，TCP端口9517
        $swoole_Server->on('request', array($this, 'huitp_json_http_hcuport_onRequest'));

        //stdxml_tcp_hcuport 传送老的标准XML消息，TCP端口9501
        $stdxml_tcp_hcuport = $swoole_Server->listen("0.0.0.0", MFUN_SWOOLE_SOCKET_STDXML_TCP_HCUPORT, SWOOLE_SOCK_TCP);
        ...
        $tcp_uiport = $swoole_Server->listen("0.0.0.0", MFUN_SWOOLE_SOCKET_STDXML_TCP_UIPORT, SWOOLE_SOCK_TCP);
        ...
        //huitpxml_tcp_hcuport, 传送HUITP XML消息，TCP端口9511
        $huitpxml_tcp_hcuport = $swoole_Server->listen("0.0.0.0", MFUN_SWOOLE_SOCKET_HUITPXML_TCP, SWOOLE_SOCK_TCP);
        ...
        //huitpjson_tcp_port, TCP端口9516
        $huitpjson_tcp_port = $swoole_Server->listen("0.0.0.0", MFUN_SWOOLE_SOCKET_HUITPJSON_TCP, SWOOLE_SOCK_TCP);
        $huitpjson_tcp_port->set(array(
            'heartbeat_check_interval' => 120,  //设置心跳检测间隔，每隔多久轮循一次，单位为秒。每次检测时遍历所有连接，如果某个连接在间隔时间内没有数据发送，则强制关闭连接（会有onClose回调）。
            'heartbeat_idle_time' => 600, //设置某个连接允许的最大闲置时间,如果某个连接在heartbeat_idle_time时间内没有数据发送，则强制关闭连接。
            'open_cpu_affinity' => true, //启用CPU亲和性设置, 在多核的硬件平台中，启用此特性会将swoole的reactor线程/worker进程绑定到固定的一个核上。可以避免进程/线程的运行时在多个核之间互相切换，提高CPU Cache的命中率。
            'reactor_num' => 8, //指定Reactor线程数,通过此参数来调节poll线程的数量，以充分利用多核,reactor_num和writer_num默认设置为CPU核数
        ...
        //ZXGT NB-IOT接入第三方设备 UDP端口9530
        $tcp_uiport = $swoole_Server->listen("0.0.0.0", MFUN_SWOOLE_SOCKET_GTJY_UDP, SWOOLE_SOCK_UDP);
        ...
        //CCL图片传输端口 TCP端口9550
        $huitpxml_tcp_picport = $swoole_Server->listen("0.0.0.0", MFUN_SWOOLE_SOCKET_DATA_STREAM_TCP, SWOOLE_SOCK_TCP);
    ...
    public function swoole_server_onStart($swoole_server) {
        global $argv;
        swoole_set_process_name("php {$argv[0]}: master");
        echo date('Y/m/d H:i:s', time())." ";
        echo "swoole_server_onStart: Master_Pid = {$swoole_server->master_pid} | "."Swoole_Version = [" . SWOOLE_VERSION . "]".PHP_EOL;
        //$swoole_server->addtimer(1000);
        ...
        if (!$mysqli) {
            echo date('Y/m/d H:i:s', time())." ";
            echo "[ERROR]swoole_server_onStart: Unable to connect to MySQL. ";
        ...
        if ($result == false){
            echo date('Y/m/d H:i:s', time())." ";
            echo "[ERROR]swoole_server_onStart: Update socketid to 0 failed!".PHP_EOL;
        }
        $mysqli->close();
    }

    public function swoole_server_onManagerStart($swoole_server) {
        global $argv;
        swoole_set_process_name("php {$argv[0]}: manager");
        echo date('Y/m/d H:i:s', time())." ";
        echo "swoole_server_onManagerStart: Manager_pid = {$swoole_server->manager_pid}".PHP_EOL;
    }

    public function swoole_server_onWorkerStart($swoole_server, $worker_id)
    {
        global $argv;
        if($worker_id >= $swoole_server->setting['worker_num'])
            swoole_set_process_name("php {$argv[0]}: task_worker");
        else
            swoole_set_process_name("php {$argv[0]}: worker");

        echo date('Y/m/d H:i:s', time())." ";
        echo "swoole_server_onWorkerStart: worker_num={$swoole_server->setting['worker_num']} | worker_pid={$swoole_server->worker_pid} | worker_id=$worker_id".PHP_EOL;
        //$serv->addtimer(500); //500ms
    }

    public function swoole_server_onWorkerStop($swoole_server, $worker_id)
    {
        echo date('Y/m/d H:i:s', time())." ";
        echo "swoole_server_onWorkerStop: [$worker_id] | worker_pid=".posix_getpid().PHP_EOL;
    }

    public function swoole_server_onFinish($swoole_server, $task_id, $data)
    {
        echo date('Y/m/d H:i:s', time())." ";
        echo "swoole_server_onFinish: task_id={$task_id} | task_data_resp={$data}".PHP_EOL;
    }

    public function swoole_server_onTask($swoole_server, $task_id, $from_id, $sql)
    {
        static $link = null;
        if ($link == null) {
            ...
            if (!$link) {
                $link = null;
                $swoole_server->finish("ER:" . mysqli_error($link));
                return;
            }
        } else {
        ...
        switch ($command){
            case "UPDATE":
                $swoole_server->finish("OK:".$link->affected_rows);
                $link->close();
                break;
            case "SELECT":
                $i=0;
                ...
                $result->free();
                $swoole_server->finish("OK:" . serialize($data));
                $link->close();
                break;
        }
        return;
    ...
    //具体port处理函数
    public function stdxml_tcp_hcuport_onConnect($swoole_server, $fd, $from_id ) {
        echo date('Y/m/d H:i:s', time())." ";
        echo "stdxml_tcp_hcuport_onConnect: HCU_Client [{$fd}] connected".PHP_EOL;
        //$swoole_server->send( $fd, "Hello {$fd}!" );
    }

    //STDXML hcuport入口函数，收到消息直接转发给HCU IOT模块并带上socketid，L1socket模块只负责消息收发，不进行任何消息解码工作
    public function stdxml_tcp_hcuport_onReceive($swoole_server, $fd, $reactor_id, $data)
    {
    ...
    public function stdxml_tcp_hcuport_onClose($swoole_server, $fd, $reactor_id)
    {
        ...
        $result = $swoole_server->taskwait($query);
        if ($result == true) {
    ...
    public  function tcp_uiport_onConnect($swoole_server, $fd, $from_id)
    {
        //正常情况不再打印
        //echo date('Y/m/d H:i:s', time())." ";
    ...
    public function tcp_uiport_onReceive($swoole_server, $fd, $reactor_id, $data)
    {
        //$swoole_server->send($fd, $data);//临时增加，等协商好之后修改
        ...
        //$swoole_server->send($fd, var_export(unserialize($db_res), true) .Y/m/d h:i:s a "\n");
        ...
        if ($socketid != 0){
            $result = $swoole_server->send($socketid, $respData);
            if ($result == false){
                echo date('Y/m/d H:i:s', time())." ";
    ...
    public function tcp_uiport_onClose($swoole_server, $fd, $reactor_id)
    {
        //正常情况不再打印
        //echo date('Y/m/d H:i:s', time())." ";
    ...
    public function huitpxml_tcp_hcuport_onConnect($swoole_server, $fd, $from_id)
    {
        echo date('Y/m/d H:i:s', time())." ";
    ...
    public function huitpxml_tcp_hcuport_onReceive($swoole_server, $fd, $reactor_id, $data)
    {
        //echo PHP_EOL.date('Y/m/d H:i:s', time())." ";
    ...
    public function huitpxml_tcp_hcuport_onClose($swoole_server, $fd, $reactor_id)
    {
        //reset socketid in t_l2sdk_iothcu_inventory when connection closed.
        $query="UPDATE t_l2sdk_iothcu_inventory  SET socketid = 0 WHERE socketid = $fd";
        $result = $swoole_server->taskwait($query);
    ...
    public function huitpjson_tcp_hcuport_onConnect($swoole_server, $fd, $from_id)
    {
        echo date('Y/m/d H:i:s', time())." ";
    ...
    //HUITP cclport入口函数，收到消息直接转发给HUITP IOT模块并带上socketid，L1socket模块只负责消息收发，不进行任何消息解码工作
    public function huitpjson_tcp_hcuport_onReceive($swoole_server, $fd, $reactor_id, $data)
    {
        echo PHP_EOL.date('Y/m/d H:i:s', time())." ";
        echo "huitpjson_tcp_hcuport_onReceive: From HCU_Client [{$fd}] : {$data}".PHP_EOL;
    ...
    public function huitpxml_tcp_picport_onConnect($swoole_server, $fd, $from_id ) {
        echo date('Y/m/d H:i:s', time())." ";
        echo "huitpxml_tcp_picport_onConnect: HCU_Client [{$fd}] connected".PHP_EOL;
        //$swoole_server->send( $fd, "Hello {$fd}!" );
    }

    public function huitpxml_tcp_picport_onReceive($swoole_server, $fd, $reactor_id, $data)
    {
        //echo PHP_EOL.date('Y/m/d H:i:s', time())." ";
    ...
    public function huitpxml_tcp_picport_onClose( $swoole_server, $fd, $from_id )
    {
        //reset socketid in t_l2sdk_iothcu_inventory when connection closed.
        $query="UPDATE t_l2sdk_iothcu_inventory  SET socketid = 0 WHERE socketid = $fd";
        $result = $swoole_server->taskwait($query);
    ...
    //具体port处理函数
    public function udp_gtjyport_onConnect($swoole_server, $fd, $from_id ) {
        echo date('Y/m/d H:i:s', time())." ";
    ...
    public function udp_gtjyport_onClose($swoole_server, $fd, $reactor_id)
    {
        echo date('Y/m/d H:i:s', time())." ";
