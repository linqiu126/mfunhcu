...
class classDbiL3apF4icm
{
    ...
    public function dbi_adjust_camera_vertical($statCode, $adj)
    {
        //建立连接
        $mysqli = new mysqli(MFUN_CLOUD_DBHOST, MFUN_CLOUD_DBUSER, MFUN_CLOUD_DBPSW, MFUN_CLOUD_DBNAME_L1L2L3, MFUN_CLOUD_DBPORT);
        if (!$mysqli) {
        ...
        $ctrl_url = "http://ngrok.hkrob.com:".$port."/ISAPI/PTZCtrl/channels/1/continuous";
        $username = MFUN_HCU_AQYC_CAM_USERNAME;
        $password = MFUN_HCU_AQYC_CAM_PASSWORD;
        if($adj == "1"){
        ...
        curl_setopt($curl, CURLOPT_POSTFIELDS, $start_xml_string);
        curl_setopt($curl, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($curl, CURLOPT_HTTPAUTH, CURLAUTH_ANY);
        curl_setopt($curl, CURLOPT_USERPWD, "$username:$password");
        ...
        curl_exec($curl);
        //停止
        $stop_xml_string = "<PTZData version=\"2.0\" xmlns=\"http://www.hikvision.com/ver20/XMLSchema\">
                             <pan>0</pan>
                             <tilt>0</tilt>
                             </PTZData>";
        curl_setopt($curl, CURLOPT_POSTFIELDS, $stop_xml_string);
        curl_exec($curl);
        curl_close($curl);
    ...
    public function dbi_adjust_camera_horizon($statCode, $adj)
    {
        //建立连接
        $mysqli = new mysqli(MFUN_CLOUD_DBHOST, MFUN_CLOUD_DBUSER, MFUN_CLOUD_DBPSW, MFUN_CLOUD_DBNAME_L1L2L3, MFUN_CLOUD_DBPORT);
        if (!$mysqli) {
