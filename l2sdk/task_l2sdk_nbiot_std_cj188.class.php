...
class classTaskL2sdkNbiotStdCj188
{
    ...
    public function mfun_l2sdk_nbiot_std_cj188_task_main_entry($parObj, $msgId, $msgName, $msg)
    {
        ...
        //入口消息内容判断
        if (empty($msg) == true) {
            $log_content = "E: NBIOT_CJ188 receive null message body";
            $loggerObj->mylog($project,"NULL","MFUN_TASK_VID_L1VM_SWOOLE","MFUN_TASK_ID_L2SDK_NBIOT_STD_CJ188",$msgName,$log_content);
        ...
        if (!empty($resp)) {
            ...
            $loggerObj->mylog($project,"NULL","MFUN_TASK_ID_L2SDK_NBIOT_STD_CJ188","NULL",$msgName,$log_content);
            echo trim($log_content); //这里echo主要是为了swoole log打印，帮助查找问题
        }
